...
export default function Header() {
    ...
    return (
        <header className='relative px-4 py-4 md:py-6 bg-white'>
            <div className="container mx-auto flex justify-between gap-4">
                <nav className={`flex-1 flex flex-col lg:flex-row lg:items-center gap-2 z-50 absolute lg:static top-0 w-full min-h-screen lg:min-h-max ${navshow ? 'right-0' : 'right-full'}`}>
                    <div onClick={() => setNavshow(!navshow)} className="lg:hidden absolute top-0 left-0 w-full h-full bg-text/50"></div>
                ...
                <div className="flex-1 flex lg:justify-center items-center flex-shrink-0">
                    <Link href={appRoutePaths.home} className='text-primary text-3xl md:text-5xl font-medium font-eugusto'>Al-Ameen</Link>
                </div>
                <div className='flex-1 flex-shrink-0 flex justify-end items-center gap-4'>
                    ...
                    <div onClick={() => setNavshow(!navshow)} className="relative lg:hidden bg-text/10 text-primary text-3xl cursor-pointer w-9 h-9 grid place-items-center font-extrabold rounded-md">
                        <HiMenuAlt3 />
                    </div>
                </div>
            </div>
